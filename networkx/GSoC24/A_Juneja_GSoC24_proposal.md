...
3. Adding get_chunks kwarg to the 5 algorithms(`number_of_isolates`, `local_efficiency`, `closeness_vitality`, `is_reachable` and `tournament_is_strongly_connected`) in nx-parallel that currently don’t have it and adding additional tests for the same. (like done in [PR#29](https://github.com/networkx/nx-parallel/pull/29) for `betweenness_centrality`)
4. adding more parallel algorithms:
    - `boruvka_mst_edges`
    - `average_clustering`
    - `approximate_average_clustering`
    - `transitive_closure_dag`
    - `compute_v_structures`
5. Expanding on the current `README` and `Contributing.md` with more details on the nx-parallel’s usage and installation, and on how the config and dispatcher are set up and works inside nx-parallel.
6. Having a decorator or something else for automatic graph conversion, instead of checking and converting the graph in all the functions, like this:
...
### Experimental and more(if time permits)
...
3. Updating the `ParalleGraph` object. Currently, nx.Graph object needs to be passed in `nxp.ParallelGraph()` to create a parallel graph instance and something like `H = nxp.ParallelGraph()` gives a `TypeError: ParallelGraph.__init__() missing 1 required positional argument: 'graph_object'`. Various operations on an `nx.Graph` object, like, [`add_weighted_edges_from`](https://github.com/networkx/networkx/blob/76c3e9bc00fb7d159b00e4c71c7274d185fcb254/networkx/classes/graph.py#L1049), [`clear_edges`](https://github.com/networkx/networkx/blob/76c3e9bc00fb7d159b00e4c71c7274d185fcb254/networkx/classes/graph.py#L1542), etc. could also be parallelized but then using such a `ParallelGraph` in nx-parallel’s parallel implementations might lead to the same issue of slowing down because of too many parallel processes, as mentioned in the 2nd point of the “Things to revisit” section.
4. Adding benchmarks based on the memory usage by multiple CPU cores.
5. cleaning up processes once the parallel algorithm is done running and seeing if that improves the performance(both in terms of time and memory) in any way. (ref. [issue](https://github.com/joblib/joblib/issues/945))
6. Adding a “shared resource-based” algorithm (using threading backend) for the future, to observe the improvements, if GIL gets removed in Python’s next release and having threads would actually improve the speedups.
7. Experimenting with default values: Using bellman-ford as the method (instead of the default dijkstra) in the `all_pairs_all_shortest_paths` algorithm improves speedups in nx-cugraphs. So, I wanted to try and see if that’s also the case in nx-parallel.
8. Adding a tutorial NB or setting up a Sphinx website for nx-parallel
9. A more comprehensive heatmap(or any other visual for showcasing speedups) for the nx-parallel’s README.md (inspiration from nx-cugraph and nx-graphblas)

### Development branches

[Just a suggested development branch structure; can be discussed and updated further]
4 Branches:

- Adding config(1st pt. in “Things to expand”)
- All benchmark-related things (2nd pt. in “Things to expand”)
- All the “revisiting” things
- All the “expanding” things (except config and benchmarks)

## 3. Schedule of deliverables

### Community bonding period(May 1 - 26)

- Things in the “Revisiting” section above(first 4 points)
...
- The rest of the things in the “Revisiting” section above
- Benchmarks(revisiting and improving them)

### Phase 2(July 12 - August 19)

- Benchmarks(running in GitHub workflows, for joblib parameters, on SNAP datasets)
- The rest of the things in the “Expanding” section(other than config and benchmarks)
